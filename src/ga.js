...
class GeneticAlgorithm {
  constructor(
    ...
    this.eliteCount = this._clampEliteCount(eliteCount);
    this.generation = 0;
    this.bestFitness = 0;
    this.avgFitness = 0;
  }
   ...
   */
  setParameters(mutationRate, crossoverRate, eliteCount = this.eliteCount) {
    this.mutationRate = mutationRate;
    this.crossoverRate = crossoverRate;
    this.eliteCount = this._clampEliteCount(eliteCount);
  }
  ...
  _clampEliteCount(n) {
    const maxElites = Math.min(3, Math.max(0, Math.floor(n || 0)));
    // Also ensure it can't exceed populationSize-1 to keep room for new offsprings
    return Math.min(maxElites, Math.max(0, this.populationSize - 1));
  }
  ...
  evolve(ants, canvasWidth, canvasHeight) {
    ...
    // Elitism: keep top N ants
    const sorted = [...ants].sort((a, b) => b.fitness - a.fitness);
    const elitesToCopy = this._clampEliteCount(this.eliteCount);
    for (let i = 0; i < elitesToCopy; i++) {
      const elite = sorted[i];
      const eliteClone = new Ant(
        Math.random() * canvasWidth,
        Math.random() * canvasHeight,
        "ga",
        elite.brain.copy()
      );
      newAnts.push(eliteClone);
    }
