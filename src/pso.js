...
class ParticleSwarmOptimization {
    constructor(populationSize = 20, inertiaWeight = 0.7, cognitiveWeight = 1.5, socialWeight = 1.5) {
    ...
    initialize(ants) {
        ...
        for (let ant of ants) {
            ...
            this.personalBestFitness.push(0);
        }
    ...
    updateParticle(particleIndex, currentWeights) {
        ...
        for (let i = 0; i < currentWeights.length; i++) {
            ...
            const cognitive = this.cognitiveWeight * Math.random() * (personalBest[i] - currentWeights[i]);
            const social = this.socialWeight * Math.random() * (globalBest[i] - currentWeights[i]);
